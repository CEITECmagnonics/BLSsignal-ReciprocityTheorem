...
FileName='StrucX'; % polarization in x
[Data,f]=DataLoader(FileName);

FileName='RefX'; % reference (without structures), adding the reference to Data
tmp=DataLoader(FileName);
Data=cat(1,Data,tmp);

FileName='StrucY'; % polarization in y
Data2=DataLoader(FileName);

FileName='RefY'; % reference (without structures), adding the reference to Data2
tmp=DataLoader(FileName);
...
fm = 301; % number of frequency points

kmax = 200; % maximal wavenumber in both x and y
...
Bext = 0.55; % external field
d = 27.55e-9; % NiFe thickness
...
Fig=figure(10); % plot of the magnon dispersion for a chosen magnon frequency
...
WindowSize=2e-6; % size of the window in which the overlap integral is evaluated
Ei = cell(M,1);
Ej = cell(M,1);
...
Fig=figure(20); %plot of the maps that highlight the ability of the system to access magnons at different kms
    Fig.OuterPosition=[0 0 1550 1550];
